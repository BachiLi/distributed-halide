...
$(BIN_DIR)/libHalide.a: $(OBJECTS) $(INITIAL_MODULES)
	...
	$(CXX) -o /dev/null -shared $(OBJECTS) $(INITIAL_MODULES) -Wl,-t $(LLVM_STATIC_LIBS) -ldl -lz -lpthread -lncurses | grep libLLVM | sed "s/[()]/ /g" > $(BUILD_DIR)/llvm_objects/list
...
DISTRIBUTED_TESTS := $(shell ls $(ROOT_DIR)/test/distributed/*.cpp)
...
test_distributed: $(DISTRIBUTED_TESTS:$(ROOT_DIR)/test/distributed/%.cpp=distributed_%)
